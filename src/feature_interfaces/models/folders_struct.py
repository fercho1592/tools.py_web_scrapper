...
class MangaFoldersStruct:
    def __init__(self, *manga_folder_path: list[str]) -> None:
        ...
        self.download_folder = FolderPath(
            root_path=fix_path(WORKING_FOLDER, "download"),
            relative_path=fix_path(WORKING_FOLDER, "download", *manga_folder_path),
        )
        #  - for images converted
        self.converted_folder = FolderPath(
            root_path=fix_path(WORKING_FOLDER, "converted"),
            relative_path=fix_path(WORKING_FOLDER, "converted", *manga_folder_path),
        )
        #  - for final pdf and
        self.pdf_folder = FolderPath(
            root_path=fix_path(WORKING_FOLDER, "pdfs"),
            relative_path=fix_path(WORKING_FOLDER, "pdfs", *manga_folder_path, ".."),
        )
        #  - for final pdf and
        ...
        self.error_log_folder = FolderPath(
            root_path=fix_path(WORKING_FOLDER, "error_logs"),
            relative_path=fix_path(WORKING_FOLDER, "error_logs", *manga_folder_path),
        )
