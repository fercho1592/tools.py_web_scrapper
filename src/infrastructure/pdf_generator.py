...
class PdfCreator(IPdfCreator):
    def __init__(
        self, imageEditor: IImageEditorService, logger: LoggerProtocol
    ) -> None:
    ...
    def CreatePdf(
        ...
        for image in fileManager.GetImagesInFolder(imageFolder):
            ...
            pdf_creator.add_page(
                "P", self.ImageEditor.get_image_size(imageFolder, image), False
            )
        ...
        fileManager.CreateIfNotexist(resultFolder)
    ...
    def add_metadata_to_pdf(self, pdf: FPDF, manga_data: dict[str, str] | None) -> None:
        ...
        for key, data in manga_data.items():
            ...
            pdf.write(
                10,
                FixStringsTools.NormalizeString(data),
            )
