...
from manga.manga_implementations._base_strategy import BaseStrategy
from manga.manga_implementations.e_web.e_web_index import EMangaIndex
from manga.manga_implementations.e_web.e_web_page import EMangaPage
from utils.custom_decorators import delayed_view_timer
...
class EMangaStrategy(BaseStrategy, IMangaStrategy):
    ...
    def get_index_page_async(self, index_page=0) -> IMangaIndex:
        dom_reader = self.HttpService.GetDoomComponentFromUrl(
            f"{self.WebPage}?p={index_page}"
        )
