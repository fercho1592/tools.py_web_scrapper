...
class BaseStrategy(ABC):
    def __init__(
        self, web_page: str, logger: LoggerProtocol, httpService: IHttpService
    ):
        self._logger = logger
...
class BaseMangaPage(ABC):
    ...
    def get_image_name(self) -> str:
        ...
        zero_array = [
            "0" for _ in range(len(str(max_image_count)) - len(str(image_number)))
        ]
