...
class HtmlElement(IWebElementDriver):
    ...
    def has_attr(self, attr: CommonAttrs, value: str | None = None) -> bool:
        return attr.value in self.Attrs and (
            value is None or self.Attrs[attr.value] == value
        )
    ...
    def has(
        ...
        if self.Tag == tag_name.value and attr is None:
            return True
        elif self.Tag == tag_name.value and (
            attr is not None and self.has_attr(attr, value)
        ):
            return True
        return False
